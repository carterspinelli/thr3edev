...
function LogoColumn({ logos, columnIndex, currentTime }: LogoColumnProps) {
  ...
  return (
    <motion.div
      className="relative h-10 w-16 overflow-hidden xs:h-12 xs:w-20 sm:h-14 sm:w-24 md:h-24 md:w-48"
        ...
        >
          <img
            ...
            className={`h-auto w-auto object-contain ${
              isMobile ? "max-h-[95%] max-w-[95%]" : "max-h-[80%] max-w-[80%]"
            }`}
          />
        </motion.div>
      </AnimatePresence>
...
export function LogoCarousel({ columns = 2, logos }: LogoCarouselProps) {
  ...
  // En dispositivos móviles, podemos usar una sola columna si es necesario
  const effectiveColumns = isMobile && columns > 2 ? 2 : columns;
  ...
  return (
    <div className={`flex justify-center ${isMobile ? 'gap-1 py-2' : 'gap-4 py-8'}`}>
